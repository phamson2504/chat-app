import React, { useState, useRef } from 'react'
import Close from '../../Assets/close.svg';
import "./ImgAvatar.css";
import authHeader from '../../Service/auth-header';
import { setAvatar } from '../../Untils/APIRoutes';
import axios from 'axios';

function ImgAvatar({ isOpen, onClose, errorForAuth }) {

    const [image, setImage] = useState(null);
    const [imageSever, setimageSever] = useState();
    const inputFileRef = useRef();

    if (!isOpen) return null;

    const handleImageChange = (event) => {
        const file = event.target.files[0];
        const reader = new FileReader();
        reader.onload = () => {
            setImage(reader.result);
            setimageSever(file);
        };
        reader.readAsDataURL(file);
    }
    const handleImageClick = () => {
        inputFileRef.current.click();
    };

    const saveAvatar = async () => {
        const formData = new FormData()
        formData.append('file', imageSever)
        const { data } = await axios.post(setAvatar, formData ,{ headers: authHeader()})
        if (data.auth === false) {
            errorForAuth();
        } else {

        }
    }

    return (
        <div className="imgAvatarOverlay">
            <div className="imgAvatarModal">
                <div className="imgAvatarClosebutton" onClick={onClose}><img src={Close} alt="logo" /></div>
                <div className="imgAvatarCloseImg">
                    <input
                        type="file"
                        accept="image/*"
                        onChange={handleImageChange}
                        style={{ display: 'none' }}
                        ref={inputFileRef}
                    />
                    {!image ?
                        (<>
                            <img src='data:image/jpeg;base64,' alt='avatar' onClick={handleImageClick} />
                        </>) :
                        (<>
                            <img src={image} alt='avatar' onClick={handleImageClick} />
                        </>)}

                </div>
                <button onClick={saveAvatar}>Save</button>
            </div>
        </div>
    );
}

export default ImgAvatar